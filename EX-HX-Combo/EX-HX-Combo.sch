EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A3 16535 11693
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L power:VCC #PWR0101
U 1 1 60D0F68F
P 1400 1200
F 0 "#PWR0101" H 1400 1050 50  0001 C CNN
F 1 "VCC" H 1415 1373 50  0000 C CNN
F 2 "" H 1400 1200 50  0001 C CNN
F 3 "" H 1400 1200 50  0001 C CNN
	1    1400 1200
	1    0    0    -1  
$EndComp
Wire Wire Line
	1400 1450 1600 1450
$Comp
L power:GND #PWR0102
U 1 1 60D0F690
P 1500 4450
F 0 "#PWR0102" H 1500 4200 50  0001 C CNN
F 1 "GND" H 1505 4277 50  0000 C CNN
F 2 "" H 1500 4450 50  0001 C CNN
F 3 "" H 1500 4450 50  0001 C CNN
	1    1500 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	1600 4250 1500 4250
Wire Wire Line
	1500 4250 1500 4450
Wire Wire Line
	1600 1250 1500 1250
Connection ~ 1500 4250
Wire Wire Line
	1600 4050 1400 4050
Wire Wire Line
	1400 4050 1400 1450
Text GLabel 2700 1350 2    50   Input ~ 0
D7
Text GLabel 2700 1450 2    50   Input ~ 0
D6
Text GLabel 2700 1550 2    50   Input ~ 0
D5
Text GLabel 2700 1650 2    50   Input ~ 0
D4
Text GLabel 2700 1750 2    50   Input ~ 0
D3
Text GLabel 2700 1850 2    50   Input ~ 0
D2
Text GLabel 2700 1950 2    50   Input ~ 0
D1
Text GLabel 2700 2050 2    50   Input ~ 0
D0
Text GLabel 2700 4250 2    50   Input ~ 0
A00
Connection ~ 1400 1450
Wire Wire Line
	1400 1200 1400 1450
Wire Wire Line
	2600 1350 2700 1350
Wire Wire Line
	2600 1450 2700 1450
Wire Wire Line
	2700 1550 2600 1550
Wire Wire Line
	2600 1650 2700 1650
Wire Wire Line
	2600 1750 2700 1750
Wire Wire Line
	2600 1850 2700 1850
Wire Wire Line
	2600 1950 2700 1950
Wire Wire Line
	2700 2050 2600 2050
Wire Wire Line
	2600 2350 2700 2350
Wire Wire Line
	2600 2450 2700 2450
Wire Wire Line
	2700 2550 2600 2550
Wire Wire Line
	2600 2650 2700 2650
Wire Wire Line
	2600 2750 2700 2750
Wire Wire Line
	2600 2950 2700 2950
Wire Wire Line
	2700 3050 2600 3050
Wire Wire Line
	2600 3150 2700 3150
Wire Wire Line
	2600 3250 2700 3250
Wire Wire Line
	2600 3350 2700 3350
Wire Wire Line
	2600 3450 2700 3450
Wire Wire Line
	2700 3550 2600 3550
Wire Wire Line
	2600 3650 2700 3650
Wire Wire Line
	2600 3750 2700 3750
Wire Wire Line
	2600 3850 2700 3850
Wire Wire Line
	2600 3950 2700 3950
Wire Wire Line
	2700 4050 2600 4050
Wire Wire Line
	2600 4150 2700 4150
Wire Wire Line
	2600 4250 2700 4250
Text GLabel 2700 4150 2    50   Input ~ 0
A01
Text GLabel 2700 4050 2    50   Input ~ 0
A02
Text GLabel 2700 3950 2    50   Input ~ 0
A03
Text GLabel 2700 3550 2    50   Input ~ 0
A07
Text GLabel 2700 3450 2    50   Input ~ 0
A08
Text GLabel 2700 3350 2    50   Input ~ 0
A09
Text GLabel 2700 3250 2    50   Input ~ 0
A10
Text GLabel 2700 3150 2    50   Input ~ 0
A11
Text GLabel 2700 3050 2    50   Input ~ 0
A12
Text GLabel 2700 2950 2    50   Input ~ 0
A13
Wire Wire Line
	1500 1250 1500 2150
Text Notes 650  750  0    79   ~ 0
TANDY PLUS BUS
Wire Notes Line
	600  600  600  4950
Wire Notes Line
	600  4950 3350 4950
Wire Notes Line
	3350 4950 3350 600 
Wire Notes Line
	3350 600  600  600 
Wire Wire Line
	1600 2150 1500 2150
Connection ~ 1500 2150
Wire Wire Line
	1500 2150 1500 4250
$Comp
L Memory_Flash:SST39SF040 U1
U 1 1 603B5177
P 13850 2400
F 0 "U1" H 13850 3971 50  0000 C CNN
F 1 "BIOS FLASH" H 13850 3880 50  0000 C CNN
F 2 "Package_DIP:DIP-32_W15.24mm_Socket" H 13850 2700 50  0001 C CNN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/25022B.pdf" H 13850 2700 50  0001 C CNN
F 4 "SST39SF010, SST39SF020, SST39SF040 or similar" H 13850 3789 50  0000 C CNN "Note"
	1    13850 2400
	1    0    0    -1  
$EndComp
NoConn ~ 11200 4450
NoConn ~ 11200 4550
NoConn ~ 11200 4650
NoConn ~ 11200 4750
NoConn ~ 11200 4850
NoConn ~ 11200 4950
NoConn ~ 11200 5050
NoConn ~ 11200 5150
Text GLabel 10400 5150 0    50   Input ~ 0
D0_BUF
Text GLabel 10400 5050 0    50   Input ~ 0
D1_BUF
Text GLabel 10400 4950 0    50   Input ~ 0
D2_BUF
Text GLabel 10400 4850 0    50   Input ~ 0
D3_BUF
Text GLabel 10400 4750 0    50   Input ~ 0
D4_BUF
Text GLabel 10400 4650 0    50   Input ~ 0
D5_BUF
Text GLabel 10400 4550 0    50   Input ~ 0
D6_BUF
Text GLabel 10400 4450 0    50   Input ~ 0
D7_BUF
Wire Wire Line
	11200 4350 11550 4350
Wire Wire Line
	11550 4350 11550 5350
$Comp
L power:GND #PWR0106
U 1 1 603C9C07
P 11550 6600
F 0 "#PWR0106" H 11550 6350 50  0001 C CNN
F 1 "GND" H 11555 6427 50  0000 C CNN
F 2 "" H 11550 6600 50  0001 C CNN
F 3 "" H 11550 6600 50  0001 C CNN
	1    11550 6600
	1    0    0    -1  
$EndComp
Wire Wire Line
	11200 5350 11550 5350
Connection ~ 11550 5350
Wire Wire Line
	11550 5350 11550 5450
Wire Wire Line
	11200 5450 11550 5450
Connection ~ 11550 5450
Wire Wire Line
	11550 5450 11550 5550
Wire Wire Line
	11200 5550 11550 5550
Connection ~ 11550 5550
Wire Wire Line
	11200 5750 11550 5750
Connection ~ 11550 5750
Wire Wire Line
	11550 5750 11550 6250
NoConn ~ 11200 5850
NoConn ~ 11200 5950
Text GLabel 10400 6050 0    50   Input ~ 0
A01
Text GLabel 10400 5950 0    50   Input ~ 0
A02
Text GLabel 11200 6050 2    50   Input ~ 0
A03
Wire Wire Line
	11200 6250 11550 6250
Connection ~ 11550 6250
Wire Wire Line
	11550 6250 11550 6600
NoConn ~ 10400 5850
$Comp
L power:GND #PWR0107
U 1 1 603E741A
P 9800 6600
F 0 "#PWR0107" H 9800 6350 50  0001 C CNN
F 1 "GND" H 9805 6427 50  0000 C CNN
F 2 "" H 9800 6600 50  0001 C CNN
F 3 "" H 9800 6600 50  0001 C CNN
	1    9800 6600
	1    0    0    -1  
$EndComp
Wire Wire Line
	9800 6600 9800 6450
Wire Wire Line
	9800 5250 10400 5250
Text GLabel 10400 5550 0    39   Input ~ 0
~IOR~_DELAYED
Text GLabel 10400 5450 0    39   Input ~ 0
~IOW
Text GLabel 10400 4350 0    39   Input ~ 0
~RESET
Text GLabel 13250 1200 0    50   Input ~ 0
A00
Text GLabel 13250 1300 0    50   Input ~ 0
A01
Text GLabel 13250 1400 0    50   Input ~ 0
A02
Text GLabel 13250 1500 0    50   Input ~ 0
A03
Text GLabel 13250 1600 0    50   Input ~ 0
A04
Text GLabel 13250 1700 0    50   Input ~ 0
A05
Text GLabel 13250 1800 0    50   Input ~ 0
A06
Text GLabel 13250 1900 0    50   Input ~ 0
A07
Text GLabel 13250 2000 0    50   Input ~ 0
A08
Text GLabel 13250 2100 0    50   Input ~ 0
A09
Text GLabel 13250 2200 0    50   Input ~ 0
A10
Text GLabel 13250 2300 0    50   Input ~ 0
A11
Text GLabel 13250 2400 0    50   Input ~ 0
A12
Text GLabel 13250 2500 0    50   Input ~ 0
A13
Wire Wire Line
	13250 3000 12650 3000
Wire Wire Line
	12650 3000 12650 3700
$Comp
L power:GND #PWR0112
U 1 1 6042FC5C
P 12650 3700
F 0 "#PWR0112" H 12650 3450 50  0001 C CNN
F 1 "GND" H 12655 3527 50  0000 C CNN
F 2 "" H 12650 3700 50  0001 C CNN
F 3 "" H 12650 3700 50  0001 C CNN
	1    12650 3700
	1    0    0    -1  
$EndComp
Text GLabel 13250 3200 0    39   Input ~ 0
~MEMW
Text GLabel 13250 3500 0    39   Input ~ 0
~MEMR~_DELAYED
Text GLabel 13250 3400 0    39   Input ~ 0
~CE~_BIOS_THRU
Text GLabel 14450 1200 2    50   Input ~ 0
D0_BUF
Text GLabel 14450 1300 2    50   Input ~ 0
D1_BUF
Text GLabel 14450 1400 2    50   Input ~ 0
D2_BUF
Text GLabel 14450 1500 2    50   Input ~ 0
D3_BUF
Text GLabel 14450 1600 2    50   Input ~ 0
D4_BUF
Text GLabel 14450 1700 2    50   Input ~ 0
D5_BUF
Text GLabel 14450 1800 2    50   Input ~ 0
D6_BUF
Text GLabel 14450 1900 2    50   Input ~ 0
D7_BUF
Text GLabel 8200 1750 2    39   Input ~ 0
~IOR~_DELAYED
Text GLabel 2700 3650 2    50   Input ~ 0
A06
Text GLabel 2700 3750 2    50   Input ~ 0
A05
Text GLabel 2700 3850 2    50   Input ~ 0
A04
Text GLabel 7200 4000 0    50   Input ~ 0
A07
Text GLabel 7200 4100 0    50   Input ~ 0
A08
Text GLabel 7200 4200 0    50   Input ~ 0
A09
Text GLabel 7200 3900 0    50   Input ~ 0
A06
Text GLabel 7200 3800 0    50   Input ~ 0
A05
Text GLabel 7200 3700 0    50   Input ~ 0
A04
Text GLabel 2700 2350 2    50   Input ~ 0
A19
Text GLabel 2700 2450 2    50   Input ~ 0
A18
Text GLabel 2700 2550 2    50   Input ~ 0
A17
Text GLabel 2700 2650 2    50   Input ~ 0
A16
Text GLabel 2700 2750 2    50   Input ~ 0
A15
Wire Wire Line
	13250 2800 12650 2800
Wire Wire Line
	12650 2800 12650 2900
Connection ~ 12650 3000
Wire Wire Line
	13250 2900 12650 2900
Connection ~ 12650 2900
Wire Wire Line
	12650 2900 12650 3000
$Comp
L Device:C C1
U 1 1 604303F2
P 8350 7700
F 0 "C1" H 8465 7746 50  0000 L CNN
F 1 "0.1uF" H 8465 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 8388 7550 50  0001 C CNN
F 3 "~" H 8350 7700 50  0001 C CNN
	1    8350 7700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0115
U 1 1 60430E20
P 8350 7850
F 0 "#PWR0115" H 8350 7600 50  0001 C CNN
F 1 "GND" H 8355 7677 50  0000 C CNN
F 2 "" H 8350 7850 50  0001 C CNN
F 3 "" H 8350 7850 50  0001 C CNN
	1    8350 7850
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0116
U 1 1 604322CA
P 8350 7550
F 0 "#PWR0116" H 8350 7400 50  0001 C CNN
F 1 "VCC" H 8365 7723 50  0000 C CNN
F 2 "" H 8350 7550 50  0001 C CNN
F 3 "" H 8350 7550 50  0001 C CNN
	1    8350 7550
	1    0    0    -1  
$EndComp
$Comp
L Device:C C2
U 1 1 60432CBC
P 8850 7700
F 0 "C2" H 8965 7746 50  0000 L CNN
F 1 "0.1uF" H 8965 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 8888 7550 50  0001 C CNN
F 3 "~" H 8850 7700 50  0001 C CNN
	1    8850 7700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0117
U 1 1 60432CD4
P 8850 7850
F 0 "#PWR0117" H 8850 7600 50  0001 C CNN
F 1 "GND" H 8855 7677 50  0000 C CNN
F 2 "" H 8850 7850 50  0001 C CNN
F 3 "" H 8850 7850 50  0001 C CNN
	1    8850 7850
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0118
U 1 1 60432CDE
P 8850 7550
F 0 "#PWR0118" H 8850 7400 50  0001 C CNN
F 1 "VCC" H 8865 7723 50  0000 C CNN
F 2 "" H 8850 7550 50  0001 C CNN
F 3 "" H 8850 7550 50  0001 C CNN
	1    8850 7550
	1    0    0    -1  
$EndComp
Text Notes 8250 7300 0    79   ~ 0
DECOUPLING
Wire Notes Line
	8200 8150 8200 7150
Text Notes 9250 750  0    79   ~ 0
DATA BUFFERING
Text Notes 12400 750  0    79   ~ 0
BIOS
Text Notes 9250 4100 0    79   ~ 0
CF ADAPTER
Wire Notes Line
	9200 3950 12250 3950
Wire Notes Line
	12250 7050 9200 7050
Wire Notes Line
	9200 3950 9200 7050
Wire Notes Line
	12250 3850 9200 3850
Wire Notes Line
	9200 600  9200 3850
Wire Notes Line
	8200 8250 11700 8250
Text Notes 8250 8400 0    79   ~ 0
JUMPERS
NoConn ~ 10400 6250
$Comp
L 00Common:IDE_CONNECTOR_44 J1
U 1 1 603E717C
P 10800 4250
F 0 "J1" H 10800 4415 50  0000 C CNN
F 1 "IDE_CONNECTOR" H 10800 4324 50  0000 C CNN
F 2 "Connector_PinSocket_2.00mm:PinSocket_2x22_P2.00mm_Horizontal" H 10800 4250 50  0001 C CNN
F 3 "" H 10800 4250 50  0001 C CNN
	1    10800 4250
	1    0    0    -1  
$EndComp
NoConn ~ 10400 5350
NoConn ~ 10400 5650
$Comp
L power:VCC #PWR0125
U 1 1 603F3CE5
P 9900 5550
F 0 "#PWR0125" H 9900 5400 50  0001 C CNN
F 1 "VCC" H 9915 5723 50  0000 C CNN
F 2 "" H 9900 5550 50  0001 C CNN
F 3 "" H 9900 5550 50  0001 C CNN
	1    9900 5550
	1    0    0    -1  
$EndComp
Wire Wire Line
	10400 5750 9900 5750
Wire Wire Line
	9900 5750 9900 5550
Wire Wire Line
	11550 5550 11550 5750
NoConn ~ 11200 5650
NoConn ~ 2600 1250
NoConn ~ 1600 1650
Text GLabel 10400 6150 0    39   Input ~ 0
~CS1~_ATA
$Comp
L power:GND #PWR0109
U 1 1 60418B5F
P 10600 3000
F 0 "#PWR0109" H 10600 2750 50  0001 C CNN
F 1 "GND" H 10605 2827 50  0000 C CNN
F 2 "" H 10600 3000 50  0001 C CNN
F 3 "" H 10600 3000 50  0001 C CNN
	1    10600 3000
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0108
U 1 1 603F73EA
P 10600 1400
F 0 "#PWR0108" H 10600 1250 50  0001 C CNN
F 1 "VCC" H 10615 1573 50  0000 C CNN
F 2 "" H 10600 1400 50  0001 C CNN
F 3 "" H 10600 1400 50  0001 C CNN
	1    10600 1400
	1    0    0    -1  
$EndComp
$Comp
L 74xx:74LS245 U2
U 1 1 603B775F
P 10600 2200
F 0 "U2" H 10800 2950 50  0000 C CNN
F 1 "74HCT245" H 10800 2850 50  0000 C CNN
F 2 "Package_DIP:DIP-20_W7.62mm_Socket" H 10600 2200 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74LS245" H 10600 2200 50  0001 C CNN
	1    10600 2200
	1    0    0    -1  
$EndComp
Text GLabel 10100 2600 0    50   Input ~ 0
BUFF_READ
Text GLabel 8200 4000 2    50   Input ~ 0
ATA_ADDR_JUMPER
Text GLabel 2700 2850 2    50   Input ~ 0
A14
Wire Wire Line
	2600 2850 2700 2850
Text GLabel 7200 1650 0    39   Input ~ 0
~MEMR
$Comp
L 00Common:Bus_Tandy_Plus J3
U 1 1 6039F90B
P 2100 2750
F 0 "J3" H 2100 4517 50  0000 C CNN
F 1 "Bus_Tandy_Plus" H 2100 4426 50  0000 C CNN
F 2 "00Custom:TandyPlusSocket" H 2100 2750 50  0001 C CNN
F 3 "https://en.wikipedia.org/wiki/Industry_Standard_Architecture" H 2100 2750 50  0001 C CNN
	1    2100 2750
	1    0    0    -1  
$EndComp
Text Label 3000 5750 0    50   ~ 0
RTC_X2
Text Label 3000 5650 0    50   ~ 0
RTC_X1
Wire Wire Line
	3150 5850 3350 5850
Wire Wire Line
	4300 6150 4600 6150
Text GLabel 4600 6250 2    39   Input ~ 0
~CE~_BIOS_THRU
Wire Wire Line
	4300 6250 4600 6250
Text GLabel 4600 6150 2    39   Input ~ 0
~CE~_BIOS_DECODE
Wire Wire Line
	4300 6050 4600 6050
Text GLabel 4600 6050 2    39   Input ~ 0
~MEMR~_DELAYED
$Comp
L power:GND #PWR0127
U 1 1 604E4DE7
P 4500 6450
F 0 "#PWR0127" H 4500 6200 50  0001 C CNN
F 1 "GND" H 4505 6277 50  0000 C CNN
F 2 "" H 4500 6450 50  0001 C CNN
F 3 "" H 4500 6450 50  0001 C CNN
	1    4500 6450
	1    0    0    -1  
$EndComp
Wire Wire Line
	4300 5850 4500 5850
Wire Wire Line
	4500 5850 4500 6450
Connection ~ 4600 5650
Wire Wire Line
	4600 5950 4600 5650
Wire Wire Line
	4300 5950 4600 5950
$Comp
L power:GND #PWR0126
U 1 1 604E4996
P 3250 6450
F 0 "#PWR0126" H 3250 6200 50  0001 C CNN
F 1 "GND" H 3255 6277 50  0000 C CNN
F 2 "" H 3250 6450 50  0001 C CNN
F 3 "" H 3250 6450 50  0001 C CNN
	1    3250 6450
	1    0    0    -1  
$EndComp
Wire Wire Line
	2300 5950 2300 6050
Wire Wire Line
	3350 5950 2300 5950
$Comp
L Device:Battery_Cell BAT1
U 1 1 604D39C9
P 2300 6250
F 0 "BAT1" H 2418 6346 50  0000 L CNN
F 1 "CR2032" H 2418 6255 50  0000 L CNN
F 2 "Battery:BatteryHolder_Keystone_1060_1x2032" V 2300 6310 50  0001 C CNN
F 3 "~" V 2300 6310 50  0001 C CNN
	1    2300 6250
	1    0    0    -1  
$EndComp
Wire Wire Line
	2300 5750 2300 5650
Wire Wire Line
	3350 5750 2300 5750
Wire Wire Line
	2950 5650 3350 5650
Wire Wire Line
	2950 5250 2950 5650
Wire Wire Line
	2300 5250 2950 5250
Wire Wire Line
	2300 5350 2300 5250
$Comp
L Device:Crystal Y1
U 1 1 604BE2E1
P 2300 5500
F 0 "Y1" V 2254 5631 50  0000 L CNN
F 1 "32.768 KHz" V 2345 5631 50  0000 L CNN
F 2 "Crystal:Crystal_C38-LF_D3.0mm_L8.0mm_Horizontal" H 2300 5500 50  0001 C CNN
F 3 "~" H 2300 5500 50  0001 C CNN
	1    2300 5500
	0    1    1    0   
$EndComp
Wire Wire Line
	3350 6250 3150 6250
Wire Wire Line
	3150 6150 3350 6150
Connection ~ 3250 6350
Wire Wire Line
	3250 6050 3250 6350
Wire Wire Line
	3350 6050 3250 6050
Wire Wire Line
	3250 6350 3250 6450
Wire Wire Line
	3350 6350 3250 6350
$Comp
L power:GND #PWR0105
U 1 1 604B2A64
P 2300 6350
F 0 "#PWR0105" H 2300 6100 50  0001 C CNN
F 1 "GND" H 2305 6177 50  0000 C CNN
F 2 "" H 2300 6350 50  0001 C CNN
F 3 "" H 2300 6350 50  0001 C CNN
	1    2300 6350
	1    0    0    -1  
$EndComp
Wire Wire Line
	4600 5650 4600 5550
Wire Wire Line
	4300 5650 4600 5650
$Comp
L power:VCC #PWR0113
U 1 1 604AF41D
P 4600 5550
F 0 "#PWR0113" H 4600 5400 50  0001 C CNN
F 1 "VCC" H 4615 5723 50  0000 C CNN
F 2 "" H 4600 5550 50  0001 C CNN
F 3 "" H 4600 5550 50  0001 C CNN
	1    4600 5550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4400 6350 4300 6350
Wire Wire Line
	4400 5750 4400 6350
Wire Wire Line
	4300 5750 4400 5750
Text GLabel 3150 6250 0    50   Input ~ 0
D0_BUF
Text GLabel 3150 6150 0    50   Input ~ 0
A00
Text GLabel 3150 5850 0    50   Input ~ 0
A02
$Comp
L 00Common:DS1315 U4
U 1 1 604AAFBB
P 3800 5450
F 0 "U4" H 3825 5475 50  0000 C CNN
F 1 "DS1315" H 3825 5384 50  0000 C CNN
F 2 "Package_DIP:DIP-16_W7.62mm_Socket" H 3800 5450 50  0001 C CNN
F 3 "" H 3800 5450 50  0001 C CNN
	1    3800 5450
	1    0    0    -1  
$EndComp
Wire Notes Line
	12350 600  15050 600 
Wire Notes Line
	600  5050 600  6800
Text Notes 650  5350 0    79   ~ 0
REAL TIME CLOCK\n(OPTIONAL)
$Comp
L Memory_RAM:AS6C4008-55PCN U5
U 1 1 605402C7
P 13650 5600
F 0 "U5" H 13800 6750 50  0000 C CNN
F 1 "AS6C4008-55PCN" H 14100 6650 50  0000 C CNN
F 2 "Package_DIP:DIP-32_W15.24mm_Socket" H 13650 5700 50  0001 C CNN
F 3 "https://www.alliancememory.com/wp-content/uploads/pdf/AS6C4008.pdf" H 13650 5700 50  0001 C CNN
	1    13650 5600
	1    0    0    -1  
$EndComp
Text GLabel 13150 4700 0    50   Input ~ 0
A00
Text GLabel 13150 4800 0    50   Input ~ 0
A01
Text GLabel 13150 4900 0    50   Input ~ 0
A02
Text GLabel 13150 5000 0    50   Input ~ 0
A03
Text GLabel 13150 5100 0    50   Input ~ 0
A04
Text GLabel 13150 5200 0    50   Input ~ 0
A05
Text GLabel 13150 5300 0    50   Input ~ 0
A06
Text GLabel 13150 5400 0    50   Input ~ 0
A07
Text GLabel 13150 5500 0    50   Input ~ 0
A08
Text GLabel 13150 5600 0    50   Input ~ 0
A09
Text GLabel 13150 5700 0    50   Input ~ 0
A10
Text GLabel 13150 5800 0    50   Input ~ 0
A11
Text GLabel 13150 5900 0    50   Input ~ 0
A12
Text GLabel 13150 6000 0    50   Input ~ 0
A13
Text GLabel 13150 6100 0    50   Input ~ 0
A14
Text GLabel 13150 6300 0    50   Input ~ 0
A16
Text GLabel 13150 6200 0    50   Input ~ 0
A15
Text GLabel 14150 4700 2    50   Input ~ 0
D0_BUF
Text GLabel 14150 4800 2    50   Input ~ 0
D1_BUF
Text GLabel 14150 4900 2    50   Input ~ 0
D2_BUF
Text GLabel 14150 5000 2    50   Input ~ 0
D3_BUF
Text GLabel 14150 5100 2    50   Input ~ 0
D4_BUF
Text GLabel 14150 5200 2    50   Input ~ 0
D5_BUF
Text GLabel 14150 5300 2    50   Input ~ 0
D6_BUF
Text GLabel 14150 5400 2    50   Input ~ 0
D7_BUF
Text GLabel 14150 5700 2    39   Input ~ 0
~CE~_RAM_DECODE
Text GLabel 14150 5800 2    39   Input ~ 0
~MEMR~_DELAYED
Text GLabel 14150 5900 2    39   Input ~ 0
~MEMW
Text Notes 12400 4300 0    79   ~ 0
SRAM
Wire Notes Line
	12350 4150 15050 4150
Wire Notes Line
	15050 4150 15050 7050
Wire Notes Line
	15050 7050 12350 7050
Wire Notes Line
	12350 7050 12350 4150
$Comp
L power:GND #PWR0130
U 1 1 60513BF2
P 13650 6700
F 0 "#PWR0130" H 13650 6450 50  0001 C CNN
F 1 "GND" H 13655 6527 50  0000 C CNN
F 2 "" H 13650 6700 50  0001 C CNN
F 3 "" H 13650 6700 50  0001 C CNN
	1    13650 6700
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0131
U 1 1 60514347
P 13650 4500
F 0 "#PWR0131" H 13650 4350 50  0001 C CNN
F 1 "VCC" H 13665 4673 50  0000 C CNN
F 2 "" H 13650 4500 50  0001 C CNN
F 3 "" H 13650 4500 50  0001 C CNN
	1    13650 4500
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0132
U 1 1 6052F317
P 10350 7550
F 0 "#PWR0132" H 10350 7400 50  0001 C CNN
F 1 "VCC" H 10365 7723 50  0000 C CNN
F 2 "" H 10350 7550 50  0001 C CNN
F 3 "" H 10350 7550 50  0001 C CNN
	1    10350 7550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0133
U 1 1 6052F30D
P 10350 7850
F 0 "#PWR0133" H 10350 7600 50  0001 C CNN
F 1 "GND" H 10355 7677 50  0000 C CNN
F 2 "" H 10350 7850 50  0001 C CNN
F 3 "" H 10350 7850 50  0001 C CNN
	1    10350 7850
	1    0    0    -1  
$EndComp
$Comp
L Device:C C5
U 1 1 6052EE45
P 10350 7700
F 0 "C5" H 10465 7746 50  0000 L CNN
F 1 "0.1uF" H 10465 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 10388 7550 50  0001 C CNN
F 3 "~" H 10350 7700 50  0001 C CNN
	1    10350 7700
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0129
U 1 1 60559BE4
P 9850 7550
F 0 "#PWR0129" H 9850 7400 50  0001 C CNN
F 1 "VCC" H 9865 7723 50  0000 C CNN
F 2 "" H 9850 7550 50  0001 C CNN
F 3 "" H 9850 7550 50  0001 C CNN
	1    9850 7550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0128
U 1 1 60559BDA
P 9850 7850
F 0 "#PWR0128" H 9850 7600 50  0001 C CNN
F 1 "GND" H 9855 7677 50  0000 C CNN
F 2 "" H 9850 7850 50  0001 C CNN
F 3 "" H 9850 7850 50  0001 C CNN
	1    9850 7850
	1    0    0    -1  
$EndComp
$Comp
L Device:C C4
U 1 1 605597A8
P 9850 7700
F 0 "C4" H 9965 7746 50  0000 L CNN
F 1 "0.1uF" H 9965 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 9888 7550 50  0001 C CNN
F 3 "~" H 9850 7700 50  0001 C CNN
	1    9850 7700
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0120
U 1 1 6043764D
P 9350 7550
F 0 "#PWR0120" H 9350 7400 50  0001 C CNN
F 1 "VCC" H 9365 7723 50  0000 C CNN
F 2 "" H 9350 7550 50  0001 C CNN
F 3 "" H 9350 7550 50  0001 C CNN
	1    9350 7550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0119
U 1 1 60437643
P 9350 7850
F 0 "#PWR0119" H 9350 7600 50  0001 C CNN
F 1 "GND" H 9355 7677 50  0000 C CNN
F 2 "" H 9350 7850 50  0001 C CNN
F 3 "" H 9350 7850 50  0001 C CNN
	1    9350 7850
	1    0    0    -1  
$EndComp
$Comp
L Device:C C3
U 1 1 6043730F
P 9350 7700
F 0 "C3" H 9465 7746 50  0000 L CNN
F 1 "0.1uF" H 9465 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 9388 7550 50  0001 C CNN
F 3 "~" H 9350 7700 50  0001 C CNN
	1    9350 7700
	1    0    0    -1  
$EndComp
Text GLabel 8200 1850 2    50   Input ~ 0
BUFF_READ
Text GLabel 7200 3500 0    39   Input ~ 0
~IOR
Wire Notes Line
	12350 4050 12350 600 
Wire Notes Line
	15050 4050 12350 4050
Wire Notes Line
	15050 600  15050 4050
Text GLabel 8200 1650 2    39   Input ~ 0
~MEMR~_DELAYED
$Comp
L power:GND #PWR0134
U 1 1 60C19D5B
P 10000 3000
F 0 "#PWR0134" H 10000 2750 50  0001 C CNN
F 1 "GND" H 10005 2827 50  0000 C CNN
F 2 "" H 10000 3000 50  0001 C CNN
F 3 "" H 10000 3000 50  0001 C CNN
	1    10000 3000
	1    0    0    -1  
$EndComp
Wire Wire Line
	10100 2700 10000 2700
Wire Wire Line
	10000 2700 10000 3000
Text GLabel 13250 2600 0    50   Input ~ 0
A14
Text GLabel 2700 2250 2    50   Input ~ 0
AEN
Wire Wire Line
	2700 2250 2600 2250
Wire Wire Line
	5250 8150 5250 9050
Wire Wire Line
	5350 7950 5450 7950
Wire Wire Line
	5350 9150 5350 7950
Wire Wire Line
	4650 9150 5350 9150
Wire Wire Line
	5250 8150 5450 8150
Wire Wire Line
	4650 9050 5250 9050
Wire Wire Line
	5150 8050 5450 8050
Wire Wire Line
	5150 8450 5150 8050
Wire Wire Line
	4650 8450 5150 8450
Wire Wire Line
	5050 7750 5450 7750
Wire Wire Line
	5050 8050 5050 7750
Wire Wire Line
	4750 8050 5050 8050
Wire Wire Line
	4750 8350 4750 8050
Wire Wire Line
	4650 8350 4750 8350
Wire Wire Line
	5150 7850 5450 7850
Wire Wire Line
	5150 7950 5150 7850
Wire Wire Line
	4650 7950 5150 7950
Wire Wire Line
	4850 8250 5450 8250
Wire Wire Line
	4850 7850 4850 8250
Wire Wire Line
	4650 7850 4850 7850
Wire Wire Line
	4950 8350 5450 8350
Wire Wire Line
	4950 7750 4950 8350
Wire Wire Line
	4650 7750 4950 7750
Wire Wire Line
	4650 7650 5450 7650
Wire Wire Line
	7200 8350 6650 8350
Wire Wire Line
	7200 8250 6650 8250
Wire Wire Line
	7200 8150 6650 8150
Wire Wire Line
	6650 8050 7200 8050
Wire Wire Line
	7200 7950 6650 7950
Wire Wire Line
	7200 7850 6650 7850
Wire Wire Line
	6650 7750 7200 7750
Wire Wire Line
	6650 7650 7200 7650
Wire Wire Line
	7100 7550 7100 8750
Wire Wire Line
	7200 7550 7100 7550
$Comp
L power:GND #PWR0110
U 1 1 60D20247
P 7100 8750
F 0 "#PWR0110" H 7100 8500 50  0001 C CNN
F 1 "GND" H 7105 8577 50  0000 C CNN
F 2 "" H 7100 8750 50  0001 C CNN
F 3 "" H 7100 8750 50  0001 C CNN
	1    7100 8750
	1    0    0    -1  
$EndComp
$Comp
L Connector:DB9_Male CN1
U 1 1 60D20251
P 7500 7950
F 0 "CN1" H 7680 7996 50  0000 L CNN
F 1 "DB9 Male" H 7680 7905 50  0000 L CNN
F 2 "w_conn_d-sub:DB_9M" H 7500 7950 50  0001 C CNN
F 3 " ~" H 7500 7950 50  0001 C CNN
	1    7500 7950
	1    0    0    -1  
$EndComp
NoConn ~ 4650 9650
NoConn ~ 4650 9550
NoConn ~ 4650 9450
NoConn ~ 4650 8550
NoConn ~ 4650 8250
Wire Wire Line
	4750 10250 4650 10250
Wire Wire Line
	4750 10050 4750 10250
Wire Wire Line
	4650 10050 4750 10050
Wire Wire Line
	2150 10050 2150 10550
Connection ~ 2150 10050
Wire Wire Line
	2650 10050 2150 10050
Text GLabel 2650 10250 0    50   Input ~ 0
RESET
Wire Wire Line
	2150 9850 2150 10050
Connection ~ 2150 9850
Wire Wire Line
	2650 9850 2150 9850
Wire Wire Line
	2150 9750 2150 9850
Wire Wire Line
	2650 9750 2150 9750
Text GLabel 2650 9950 0    39   Input ~ 0
~IOR
Text GLabel 2650 9650 0    39   Input ~ 0
~IOW
$Comp
L power:GND #PWR0111
U 1 1 60D20286
P 2150 10550
F 0 "#PWR0111" H 2150 10300 50  0001 C CNN
F 1 "GND" H 2155 10377 50  0000 C CNN
F 2 "" H 2150 10550 50  0001 C CNN
F 3 "" H 2150 10550 50  0001 C CNN
	1    2150 10550
	1    0    0    -1  
$EndComp
Text GLabel 2650 8550 0    50   Input ~ 0
A02
Text GLabel 2650 8450 0    50   Input ~ 0
A01
Text GLabel 2650 8350 0    50   Input ~ 0
A00
$Comp
L 00Common:16550_48 U6
U 1 1 60D20293
P 3650 8850
F 0 "U6" H 3750 10550 50  0000 C CNN
F 1 "16550/16C550" H 4000 10450 50  0000 C CNN
F 2 "Package_QFP:LQFP-48_7x7mm_P0.5mm" H 3650 8850 50  0001 C CIN
F 3 "http://www.ti.com/lit/ds/symlink/pc16550d.pdf" H 3650 8850 50  0001 C CNN
	1    3650 8850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0137
U 1 1 60D2029D
P 3650 10550
F 0 "#PWR0137" H 3650 10300 50  0001 C CNN
F 1 "GND" H 3655 10377 50  0000 C CNN
F 2 "" H 3650 10550 50  0001 C CNN
F 3 "" H 3650 10550 50  0001 C CNN
	1    3650 10550
	1    0    0    -1  
$EndComp
$Comp
L power:-12V #PWR0138
U 1 1 60D202A7
P 6150 8750
F 0 "#PWR0138" H 6150 8850 50  0001 C CNN
F 1 "-12V" H 6165 8923 50  0000 C CNN
F 2 "" H 6150 8750 50  0001 C CNN
F 3 "" H 6150 8750 50  0001 C CNN
	1    6150 8750
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR0139
U 1 1 60D202B1
P 5950 8750
F 0 "#PWR0139" H 5950 8500 50  0001 C CNN
F 1 "GND" H 5955 8577 50  0000 C CNN
F 2 "" H 5950 8750 50  0001 C CNN
F 3 "" H 5950 8750 50  0001 C CNN
	1    5950 8750
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0140
U 1 1 60D202BB
P 5950 7350
F 0 "#PWR0140" H 5950 7200 50  0001 C CNN
F 1 "VCC" H 5965 7523 50  0000 C CNN
F 2 "" H 5950 7350 50  0001 C CNN
F 3 "" H 5950 7350 50  0001 C CNN
	1    5950 7350
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0141
U 1 1 60D202C5
P 3650 7150
F 0 "#PWR0141" H 3650 7000 50  0001 C CNN
F 1 "VCC" H 3665 7323 50  0000 C CNN
F 2 "" H 3650 7150 50  0001 C CNN
F 3 "" H 3650 7150 50  0001 C CNN
	1    3650 7150
	1    0    0    -1  
$EndComp
$Comp
L Interface_UART:GD75232N U7
U 1 1 60D202CF
P 6050 8050
F 0 "U7" H 6300 8750 50  0000 C CNN
F 1 "GD75232N" H 6400 8650 50  0000 C CNN
F 2 "Package_SO:SOIC-20W_7.5x12.8mm_P1.27mm" H 6050 7250 50  0001 C CIN
F 3 "http://www.ti.com/lit/ds/symlink/gd75232.pdf" H 5550 8050 50  0001 C CNN
	1    6050 8050
	1    0    0    -1  
$EndComp
$Comp
L Device:C C7
U 1 1 60D2034A
P 11400 7700
F 0 "C7" H 11515 7746 50  0000 L CNN
F 1 "0.1uF" H 11515 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 11438 7550 50  0001 C CNN
F 3 "~" H 11400 7700 50  0001 C CNN
	1    11400 7700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0142
U 1 1 60D2035E
P 11400 7850
F 0 "#PWR0142" H 11400 7600 50  0001 C CNN
F 1 "GND" H 11405 7677 50  0000 C CNN
F 2 "" H 11400 7850 50  0001 C CNN
F 3 "" H 11400 7850 50  0001 C CNN
	1    11400 7850
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0143
U 1 1 60D20368
P 11400 7550
F 0 "#PWR0143" H 11400 7400 50  0001 C CNN
F 1 "VCC" H 11415 7723 50  0000 C CNN
F 2 "" H 11400 7550 50  0001 C CNN
F 3 "" H 11400 7550 50  0001 C CNN
	1    11400 7550
	1    0    0    -1  
$EndComp
$Comp
L Device:C C8
U 1 1 60D20372
P 11900 7700
F 0 "C8" H 12015 7746 50  0000 L CNN
F 1 "0.1uF" H 12015 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 11938 7550 50  0001 C CNN
F 3 "~" H 11900 7700 50  0001 C CNN
	1    11900 7700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0144
U 1 1 60D2037C
P 11900 7850
F 0 "#PWR0144" H 11900 7600 50  0001 C CNN
F 1 "GND" H 11905 7677 50  0000 C CNN
F 2 "" H 11900 7850 50  0001 C CNN
F 3 "" H 11900 7850 50  0001 C CNN
	1    11900 7850
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0145
U 1 1 60D20386
P 11900 7550
F 0 "#PWR0145" H 11900 7400 50  0001 C CNN
F 1 "VCC" H 11915 7723 50  0000 C CNN
F 2 "" H 11900 7550 50  0001 C CNN
F 3 "" H 11900 7550 50  0001 C CNN
	1    11900 7550
	1    0    0    -1  
$EndComp
$Comp
L Device:C C9
U 1 1 60D20390
P 12400 7700
F 0 "C9" H 12515 7746 50  0000 L CNN
F 1 "0.1uF" H 12515 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 12438 7550 50  0001 C CNN
F 3 "~" H 12400 7700 50  0001 C CNN
	1    12400 7700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0146
U 1 1 60D2039A
P 12400 7850
F 0 "#PWR0146" H 12400 7600 50  0001 C CNN
F 1 "GND" H 12405 7677 50  0000 C CNN
F 2 "" H 12400 7850 50  0001 C CNN
F 3 "" H 12400 7850 50  0001 C CNN
	1    12400 7850
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0147
U 1 1 60D203A4
P 12400 7550
F 0 "#PWR0147" H 12400 7400 50  0001 C CNN
F 1 "VCC" H 12415 7723 50  0000 C CNN
F 2 "" H 12400 7550 50  0001 C CNN
F 3 "" H 12400 7550 50  0001 C CNN
	1    12400 7550
	1    0    0    -1  
$EndComp
Text GLabel 2650 8150 0    50   Input ~ 0
D7
Text GLabel 2650 8050 0    50   Input ~ 0
D6
Text GLabel 2650 7950 0    50   Input ~ 0
D5
Text GLabel 2650 7850 0    50   Input ~ 0
D4
Text GLabel 2650 7750 0    50   Input ~ 0
D3
Text GLabel 2650 7650 0    50   Input ~ 0
D2
Text GLabel 2650 7550 0    50   Input ~ 0
D1
Text GLabel 2650 7450 0    50   Input ~ 0
D0
$Comp
L power:VCC #PWR0150
U 1 1 60D203CA
P 2350 7200
F 0 "#PWR0150" H 2350 7050 50  0001 C CNN
F 1 "VCC" H 2365 7373 50  0000 C CNN
F 2 "" H 2350 7200 50  0001 C CNN
F 3 "" H 2350 7200 50  0001 C CNN
	1    2350 7200
	1    0    0    -1  
$EndComp
Wire Wire Line
	2650 8650 2350 8650
Wire Wire Line
	2350 8650 2350 7200
Text GLabel 2650 9050 0    50   Input ~ 0
UART_XIN
$Comp
L Device:C C6
U 1 1 60D2040B
P 10850 7700
F 0 "C6" H 10965 7746 50  0000 L CNN
F 1 "0.1uF" H 10965 7655 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 10888 7550 50  0001 C CNN
F 3 "~" H 10850 7700 50  0001 C CNN
	1    10850 7700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0154
U 1 1 60D20415
P 10850 7850
F 0 "#PWR0154" H 10850 7600 50  0001 C CNN
F 1 "GND" H 10855 7677 50  0000 C CNN
F 2 "" H 10850 7850 50  0001 C CNN
F 3 "" H 10850 7850 50  0001 C CNN
	1    10850 7850
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0155
U 1 1 60D2041F
P 10850 7550
F 0 "#PWR0155" H 10850 7400 50  0001 C CNN
F 1 "VCC" H 10865 7723 50  0000 C CNN
F 2 "" H 10850 7550 50  0001 C CNN
F 3 "" H 10850 7550 50  0001 C CNN
	1    10850 7550
	1    0    0    -1  
$EndComp
Text GLabel 2650 8850 0    39   Input ~ 0
~UART_CS
Wire Wire Line
	2650 8750 2350 8750
Wire Wire Line
	2350 8750 2350 8650
Connection ~ 2350 8650
Text Notes 700  7200 0    79   ~ 0
UART AND\nPORT TRANCEIVERS
Text Notes 6300 5200 0    79   ~ 0
UART CLOCK
Wire Notes Line
	600  6900 600  11100
Wire Notes Line
	600  11100 8100 11100
Wire Notes Line
	8100 11100 8100 6900
Wire Notes Line
	8100 6900 600  6900
Wire Notes Line
	6250 5050 9100 5050
Wire Notes Line
	9100 5050 9100 6800
Wire Notes Line
	9100 6800 6250 6800
Wire Notes Line
	6250 6800 6250 5050
$Comp
L power:+12V #PWR0156
U 1 1 60D204E6
P 6150 7350
F 0 "#PWR0156" H 6150 7200 50  0001 C CNN
F 1 "+12V" H 6165 7523 50  0000 C CNN
F 2 "" H 6150 7350 50  0001 C CNN
F 3 "" H 6150 7350 50  0001 C CNN
	1    6150 7350
	1    0    0    -1  
$EndComp
Text Label 6700 8350 0    50   ~ 0
SER_PIN1
Text Label 6700 8250 0    50   ~ 0
SER_PIN6
Text Label 6700 8150 0    50   ~ 0
SER_PIN2
Text Label 6700 8050 0    50   ~ 0
SER_PIN7
Text Label 6700 7950 0    50   ~ 0
SER_PIN3
Text Label 6700 7850 0    50   ~ 0
SER_PIN8
Text Label 6700 7750 0    50   ~ 0
SER_PIN4
Text Label 6700 7650 0    50   ~ 0
SER_PIN9
Wire Wire Line
	13250 2700 12650 2700
Wire Wire Line
	12650 2700 12650 2800
Connection ~ 12650 2800
NoConn ~ 2650 9350
$Comp
L 00Common:GAL16V8 U8
U 1 1 60E9494F
P 7700 1750
F 0 "U8" H 7900 2500 50  0000 C CNN
F 1 "GAL16V8" H 7900 2400 50  0000 C CNN
F 2 "Package_DIP:DIP-20_W7.62mm_Socket" H 7700 1750 50  0001 C CNN
F 3 "" H 7700 1750 50  0001 C CNN
	1    7700 1750
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0161
U 1 1 60E96D76
P 7700 1050
F 0 "#PWR0161" H 7700 900 50  0001 C CNN
F 1 "VCC" H 7715 1223 50  0000 C CNN
F 2 "" H 7700 1050 50  0001 C CNN
F 3 "" H 7700 1050 50  0001 C CNN
	1    7700 1050
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0162
U 1 1 60E9723F
P 7700 2450
F 0 "#PWR0162" H 7700 2200 50  0001 C CNN
F 1 "GND" H 7705 2277 50  0000 C CNN
F 2 "" H 7700 2450 50  0001 C CNN
F 3 "" H 7700 2450 50  0001 C CNN
	1    7700 2450
	1    0    0    -1  
$EndComp
$Comp
L 00Common:GAL16V8 U9
U 1 1 60E9865C
P 7700 3800
F 0 "U9" H 7900 4550 50  0000 C CNN
F 1 "GAL16V8" H 7900 4450 50  0000 C CNN
F 2 "Package_DIP:DIP-20_W7.62mm_Socket" H 7700 3800 50  0001 C CNN
F 3 "" H 7700 3800 50  0001 C CNN
	1    7700 3800
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0163
U 1 1 60E98FEA
P 7700 3100
F 0 "#PWR0163" H 7700 2950 50  0001 C CNN
F 1 "VCC" H 7715 3273 50  0000 C CNN
F 2 "" H 7700 3100 50  0001 C CNN
F 3 "" H 7700 3100 50  0001 C CNN
	1    7700 3100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0164
U 1 1 60E98FF4
P 7700 4500
F 0 "#PWR0164" H 7700 4250 50  0001 C CNN
F 1 "GND" H 7705 4327 50  0000 C CNN
F 2 "" H 7700 4500 50  0001 C CNN
F 3 "" H 7700 4500 50  0001 C CNN
	1    7700 4500
	1    0    0    -1  
$EndComp
Text GLabel 7200 2150 0    50   Input ~ 0
A19
Text GLabel 7200 2050 0    50   Input ~ 0
A18
Text GLabel 7200 1950 0    50   Input ~ 0
A17
Text GLabel 7200 1850 0    50   Input ~ 0
A16
Text GLabel 7200 1750 0    50   Input ~ 0
A15
Text GLabel 7200 3600 0    50   Input ~ 0
A03
Text GLabel 7200 3400 0    50   Input ~ 0
UART_S1
Text GLabel 8200 3300 2    39   Input ~ 0
~CS1~_ATA
Text GLabel 8200 3500 2    39   Input ~ 0
~UART_CS
Text GLabel 8200 1350 2    39   Input ~ 0
~CE~_RAM_DECODE
Text GLabel 8200 1550 2    50   Input ~ 0
A17_MAPPED
Text GLabel 9000 9500 0    50   Input ~ 0
BASE_RAM_SIZE
Text GLabel 9000 9800 0    50   Input ~ 0
ATA_ADDR_JUMPER
Text GLabel 8200 3600 2    50   Input ~ 0
IOR_DECODE
Text GLabel 8200 1250 2    39   Input ~ 0
~CE~_BIOS_DECODE
Text GLabel 8200 1450 2    50   Input ~ 0
A18_MAPPED
Text GLabel 7200 1250 0    50   Input ~ 0
BASE_RAM_ENABLE
Text GLabel 9000 9400 0    50   Input ~ 0
BASE_RAM_ENABLE
Text GLabel 7200 1350 0    50   Input ~ 0
BASE_RAM_SIZE
Text GLabel 7200 1450 0    50   Input ~ 0
UMB_ENABLE
Text GLabel 7200 1550 0    50   Input ~ 0
UMB_SIZE
$Comp
L power:+12V #PWR0175
U 1 1 60CE9453
P 1150 1200
F 0 "#PWR0175" H 1150 1050 50  0001 C CNN
F 1 "+12V" H 1165 1373 50  0000 C CNN
F 2 "" H 1150 1200 50  0001 C CNN
F 3 "" H 1150 1200 50  0001 C CNN
	1    1150 1200
	1    0    0    -1  
$EndComp
Wire Wire Line
	1600 2050 1150 2050
Wire Wire Line
	1150 2050 1150 1200
$Comp
L power:-12V #PWR0176
U 1 1 60CF5721
P 1250 4450
F 0 "#PWR0176" H 1250 4550 50  0001 C CNN
F 1 "-12V" H 1265 4623 50  0000 C CNN
F 2 "" H 1250 4450 50  0001 C CNN
F 3 "" H 1250 4450 50  0001 C CNN
	1    1250 4450
	-1   0    0    1   
$EndComp
Wire Wire Line
	1600 1850 1250 1850
Wire Wire Line
	1250 1850 1250 4450
Text GLabel 8200 3900 2    50   Input ~ 0
AEN
Text GLabel 13150 6500 0    50   Input ~ 0
A18_MAPPED
Text GLabel 13150 6400 0    50   Input ~ 0
A17_MAPPED
$Comp
L power:VCC #PWR0121
U 1 1 60E3DBDE
P 13400 7550
F 0 "#PWR0121" H 13400 7400 50  0001 C CNN
F 1 "VCC" H 13415 7723 50  0000 C CNN
F 2 "" H 13400 7550 50  0001 C CNN
F 3 "" H 13400 7550 50  0001 C CNN
	1    13400 7550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0122
U 1 1 60E3E530
P 13400 7850
F 0 "#PWR0122" H 13400 7600 50  0001 C CNN
F 1 "GND" H 13405 7677 50  0000 C CNN
F 2 "" H 13400 7850 50  0001 C CNN
F 3 "" H 13400 7850 50  0001 C CNN
	1    13400 7850
	1    0    0    -1  
$EndComp
Wire Notes Line
	8200 8250 8200 11100
Wire Notes Line
	8200 11100 11700 11100
Wire Notes Line
	11700 11100 11700 8250
Text Notes 6500 850  0    79   ~ 0
DECODING
Wire Notes Line
	6250 600  9100 600 
Wire Notes Line
	9100 600  9100 4950
Wire Notes Line
	9100 4950 6250 4950
Wire Notes Line
	6250 4950 6250 600 
Wire Notes Line
	6150 5050 600  5050
Wire Notes Line
	6150 6800 6150 5050
Wire Notes Line
	600  6800 6150 6800
Wire Notes Line
	9200 600  12250 600 
Wire Notes Line
	12250 3850 12250 600 
Wire Notes Line
	12250 3950 12250 7050
Text GLabel 8200 3400 2    39   Input ~ 0
~CS3~_ATA
Text GLabel 11200 6150 2    39   Input ~ 0
~CS3~_ATA
Text GLabel 9000 9600 0    50   Input ~ 0
UMB_ENABLE
Text GLabel 9000 9700 0    50   Input ~ 0
UMB_SIZE
Text GLabel 7200 3300 0    50   Input ~ 0
UART_S0
Text GLabel 8200 3800 2    50   Input ~ 0
RESET
Text GLabel 8200 3700 2    39   Input ~ 0
~RESET
Text GLabel 8200 1950 2    50   Input ~ 0
IOR_DECODE
Text GLabel 9000 9900 0    50   Input ~ 0
UART_S0
Text GLabel 9000 10000 0    50   Input ~ 0
UART_S1
Text GLabel 2650 10150 0    50   Input ~ 0
UART_IRQ
Text GLabel 11100 10100 2    50   Input ~ 0
UART_IRQ
NoConn ~ 11200 6450
Wire Wire Line
	10400 6450 9800 6450
Connection ~ 9800 6450
Wire Wire Line
	9800 6450 9800 5250
Wire Wire Line
	9900 5750 9900 6350
Wire Wire Line
	9900 6350 10400 6350
Connection ~ 9900 5750
$Comp
L power:VCC #PWR0103
U 1 1 60DBF608
P 11650 6150
F 0 "#PWR0103" H 11650 6000 50  0001 C CNN
F 1 "VCC" H 11665 6323 50  0000 C CNN
F 2 "" H 11650 6150 50  0001 C CNN
F 3 "" H 11650 6150 50  0001 C CNN
	1    11650 6150
	1    0    0    -1  
$EndComp
Wire Wire Line
	11650 6350 11650 6150
Wire Wire Line
	11650 6350 11200 6350
Text GLabel 10400 10100 0    50   Input ~ 0
IRQ3
Text GLabel 10400 10200 0    50   Input ~ 0
IRQ4
Wire Wire Line
	10400 9400 10300 9400
$Comp
L power:GND #PWR0104
U 1 1 60F1B6E7
P 11600 10500
F 0 "#PWR0104" H 11600 10250 50  0001 C CNN
F 1 "GND" H 11605 10327 50  0000 C CNN
F 2 "" H 11600 10500 50  0001 C CNN
F 3 "" H 11600 10500 50  0001 C CNN
	1    11600 10500
	1    0    0    -1  
$EndComp
Wire Wire Line
	11000 9400 11600 9400
Wire Wire Line
	11600 9400 11600 9500
Wire Wire Line
	11000 10000 11600 10000
Connection ~ 11600 10000
Wire Wire Line
	11600 10000 11600 10500
Wire Wire Line
	11000 9900 11600 9900
Connection ~ 11600 9900
Wire Wire Line
	11600 9900 11600 10000
Wire Wire Line
	11000 9800 11600 9800
Connection ~ 11600 9800
Wire Wire Line
	11600 9800 11600 9900
Wire Wire Line
	11000 9700 11600 9700
Connection ~ 11600 9700
Wire Wire Line
	11600 9700 11600 9800
Wire Wire Line
	11000 9600 11600 9600
Connection ~ 11600 9600
Wire Wire Line
	11600 9600 11600 9700
Wire Wire Line
	11000 9500 11600 9500
Connection ~ 11600 9500
Wire Wire Line
	11600 9500 11600 9600
$Comp
L power:VCC #PWR0114
U 1 1 60F8536B
P 10100 8900
F 0 "#PWR0114" H 10100 8750 50  0001 C CNN
F 1 "VCC" H 10115 9073 50  0000 C CNN
F 2 "" H 10100 8900 50  0001 C CNN
F 3 "" H 10100 8900 50  0001 C CNN
	1    10100 8900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R7
U 1 1 60F85D03
P 9100 9050
F 0 "R7" V 9000 9000 50  0000 L CNN
F 1 "10K" V 9100 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 9030 9050 50  0001 C CNN
F 3 "~" H 9100 9050 50  0001 C CNN
	1    9100 9050
	1    0    0    -1  
$EndComp
Wire Wire Line
	10300 9400 10300 9200
$Comp
L Device:R R1
U 1 1 60EE41E5
P 10300 9050
F 0 "R1" V 10200 9000 50  0000 L CNN
F 1 "10K" V 10300 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 10230 9050 50  0001 C CNN
F 3 "~" H 10300 9050 50  0001 C CNN
	1    10300 9050
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0167
U 1 1 60F6A4EE
P 10300 8900
F 0 "#PWR0167" H 10300 8750 50  0001 C CNN
F 1 "VCC" H 10315 9073 50  0000 C CNN
F 2 "" H 10300 8900 50  0001 C CNN
F 3 "" H 10300 8900 50  0001 C CNN
	1    10300 8900
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0123
U 1 1 60FB6FA7
P 9700 8900
F 0 "#PWR0123" H 9700 8750 50  0001 C CNN
F 1 "VCC" H 9715 9073 50  0000 C CNN
F 2 "" H 9700 8900 50  0001 C CNN
F 3 "" H 9700 8900 50  0001 C CNN
	1    9700 8900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R5
U 1 1 60FB795F
P 9500 9050
F 0 "R5" V 9400 9000 50  0000 L CNN
F 1 "10K" V 9500 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 9430 9050 50  0001 C CNN
F 3 "~" H 9500 9050 50  0001 C CNN
	1    9500 9050
	1    0    0    -1  
$EndComp
$Comp
L Device:R R6
U 1 1 60FB7969
P 9300 9050
F 0 "R6" V 9200 9000 50  0000 L CNN
F 1 "10K" V 9300 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 9230 9050 50  0001 C CNN
F 3 "~" H 9300 9050 50  0001 C CNN
	1    9300 9050
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0124
U 1 1 60FB7973
P 9900 8900
F 0 "#PWR0124" H 9900 8750 50  0001 C CNN
F 1 "VCC" H 9915 9073 50  0000 C CNN
F 2 "" H 9900 8900 50  0001 C CNN
F 3 "" H 9900 8900 50  0001 C CNN
	1    9900 8900
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0135
U 1 1 60FC70AE
P 9300 8900
F 0 "#PWR0135" H 9300 8750 50  0001 C CNN
F 1 "VCC" H 9315 9073 50  0000 C CNN
F 2 "" H 9300 8900 50  0001 C CNN
F 3 "" H 9300 8900 50  0001 C CNN
	1    9300 8900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R3
U 1 1 60FC7AA6
P 9900 9050
F 0 "R3" V 9800 9000 50  0000 L CNN
F 1 "10K" V 9900 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 9830 9050 50  0001 C CNN
F 3 "~" H 9900 9050 50  0001 C CNN
	1    9900 9050
	1    0    0    -1  
$EndComp
$Comp
L Device:R R4
U 1 1 60FC7AB0
P 9700 9050
F 0 "R4" V 9600 9000 50  0000 L CNN
F 1 "10K" V 9700 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 9630 9050 50  0001 C CNN
F 3 "~" H 9700 9050 50  0001 C CNN
	1    9700 9050
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0136
U 1 1 60FC7ABA
P 9500 8900
F 0 "#PWR0136" H 9500 8750 50  0001 C CNN
F 1 "VCC" H 9515 9073 50  0000 C CNN
F 2 "" H 9500 8900 50  0001 C CNN
F 3 "" H 9500 8900 50  0001 C CNN
	1    9500 8900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R2
U 1 1 60FC7AD8
P 10100 9050
F 0 "R2" V 10000 9000 50  0000 L CNN
F 1 "10K" V 10100 8950 50  0000 L CNN
F 2 "Resistor_SMD:R_1206_3216Metric" V 10030 9050 50  0001 C CNN
F 3 "~" H 10100 9050 50  0001 C CNN
	1    10100 9050
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0165
U 1 1 60FC7AE2
P 9100 8900
F 0 "#PWR0165" H 9100 8750 50  0001 C CNN
F 1 "VCC" H 9115 9073 50  0000 C CNN
F 2 "" H 9100 8900 50  0001 C CNN
F 3 "" H 9100 8900 50  0001 C CNN
	1    9100 8900
	1    0    0    -1  
$EndComp
Wire Wire Line
	10100 9200 10100 9500
Wire Wire Line
	10100 9500 10400 9500
Wire Wire Line
	9900 9200 9900 9600
Wire Wire Line
	9900 9600 10400 9600
Wire Wire Line
	9700 9200 9700 9700
Wire Wire Line
	9700 9700 10400 9700
Wire Wire Line
	9500 9200 9500 9800
Wire Wire Line
	9500 9800 10400 9800
Wire Wire Line
	9300 9200 9300 9900
Wire Wire Line
	9300 9900 10400 9900
Wire Wire Line
	10400 10000 9100 10000
Wire Wire Line
	9100 10000 9100 9200
Wire Wire Line
	10300 9400 9000 9400
Connection ~ 10300 9400
Wire Wire Line
	10100 9500 9000 9500
Connection ~ 10100 9500
Wire Wire Line
	9900 9600 9000 9600
Connection ~ 9900 9600
Wire Wire Line
	9700 9700 9000 9700
Connection ~ 9700 9700
Wire Wire Line
	9500 9800 9000 9800
Connection ~ 9500 9800
Wire Wire Line
	9300 9900 9000 9900
Connection ~ 9300 9900
Wire Wire Line
	9100 10000 9000 10000
Connection ~ 9100 10000
Wire Wire Line
	14900 7450 14900 7550
Wire Wire Line
	14450 7450 14900 7450
Wire Wire Line
	14450 7550 14450 7450
$Comp
L power:-12V #PWR0160
U 1 1 60D20518
P 14450 7850
F 0 "#PWR0160" H 14450 7950 50  0001 C CNN
F 1 "-12V" H 14465 8023 50  0000 C CNN
F 2 "" H 14450 7850 50  0001 C CNN
F 3 "" H 14450 7850 50  0001 C CNN
	1    14450 7850
	-1   0    0    1   
$EndComp
$Comp
L power:+12V #PWR0159
U 1 1 60D2050E
P 13950 7550
F 0 "#PWR0159" H 13950 7400 50  0001 C CNN
F 1 "+12V" H 13965 7723 50  0000 C CNN
F 2 "" H 13950 7550 50  0001 C CNN
F 3 "" H 13950 7550 50  0001 C CNN
	1    13950 7550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0158
U 1 1 60D20504
P 14900 7550
F 0 "#PWR0158" H 14900 7300 50  0001 C CNN
F 1 "GND" H 14905 7377 50  0000 C CNN
F 2 "" H 14900 7550 50  0001 C CNN
F 3 "" H 14900 7550 50  0001 C CNN
	1    14900 7550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0157
U 1 1 60D204FA
P 13950 7850
F 0 "#PWR0157" H 13950 7600 50  0001 C CNN
F 1 "GND" H 13955 7677 50  0000 C CNN
F 2 "" H 13950 7850 50  0001 C CNN
F 3 "" H 13950 7850 50  0001 C CNN
	1    13950 7850
	1    0    0    -1  
$EndComp
Wire Notes Line
	15050 8150 8200 8150
Wire Notes Line
	15050 7150 15050 8150
Wire Notes Line
	8200 7150 15050 7150
$Comp
L power:GND #PWR0151
U 1 1 60D203E0
P 7550 6250
F 0 "#PWR0151" H 7550 6000 50  0001 C CNN
F 1 "GND" H 7555 6077 50  0000 C CNN
F 2 "" H 7550 6250 50  0001 C CNN
F 3 "" H 7550 6250 50  0001 C CNN
	1    7550 6250
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0152
U 1 1 60D203EB
P 7150 5650
F 0 "#PWR0152" H 7150 5500 50  0001 C CNN
F 1 "VCC" H 7165 5823 50  0000 C CNN
F 2 "" H 7150 5650 50  0001 C CNN
F 3 "" H 7150 5650 50  0001 C CNN
	1    7150 5650
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0153
U 1 1 60D203F5
P 7550 5650
F 0 "#PWR0153" H 7550 5500 50  0001 C CNN
F 1 "VCC" H 7565 5823 50  0000 C CNN
F 2 "" H 7550 5650 50  0001 C CNN
F 3 "" H 7550 5650 50  0001 C CNN
	1    7550 5650
	1    0    0    -1  
$EndComp
Wire Wire Line
	7250 5950 7150 5950
$Comp
L Oscillator:SG-7050CAN U3
U 1 1 6125B3CC
P 7550 5950
F 0 "U3" H 7650 6300 50  0000 L CNN
F 1 "1.8432 MHz" H 7650 6200 50  0000 L CNN
F 2 "Oscillator:Oscillator_SMD_SeikoEpson_SG8002CA-4Pin_7.0x5.0mm" H 8250 5600 50  0001 C CNN
F 3 "https://support.epson.biz/td/api/doc_check.php?dl=brief_SG7050CAN&lang=en" H 7450 5950 50  0001 C CNN
	1    7550 5950
	1    0    0    -1  
$EndComp
Wire Wire Line
	7150 5650 7150 5950
Text GLabel 7850 5950 2    50   Input ~ 0
UART_XIN
$Comp
L Device:CP C10
U 1 1 60E3E53A
P 13400 7700
F 0 "C10" H 13518 7746 50  0000 L CNN
F 1 "100uF" H 13518 7655 50  0000 L CNN
F 2 "Capacitor_Tantalum_SMD:CP_EIA-3528-15_AVX-H" H 13438 7550 50  0001 C CNN
F 3 "~" H 13400 7700 50  0001 C CNN
	1    13400 7700
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C11
U 1 1 60D204F0
P 13950 7700
F 0 "C11" H 14068 7746 50  0000 L CNN
F 1 "10uF" H 14068 7655 50  0000 L CNN
F 2 "Capacitor_Tantalum_SMD:CP_EIA-3216-18_Kemet-A" H 13988 7550 50  0001 C CNN
F 3 "~" H 13950 7700 50  0001 C CNN
	1    13950 7700
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C12
U 1 1 60D20522
P 14450 7700
F 0 "C12" H 14568 7746 50  0000 L CNN
F 1 "10uF" H 14568 7655 50  0000 L CNN
F 2 "Capacitor_Tantalum_SMD:CP_EIA-3216-18_Kemet-A" H 14488 7550 50  0001 C CNN
F 3 "~" H 14450 7700 50  0001 C CNN
	1    14450 7700
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole_Pad H2
U 1 1 60D6AC46
P 13200 8750
F 0 "H2" V 13154 8900 50  0000 L CNN
F 1 "MountingHole_Pad" V 13245 8900 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.5mm_Pad" H 13200 8750 50  0001 C CNN
F 3 "~" H 13200 8750 50  0001 C CNN
	1    13200 8750
	0    1    1    0   
$EndComp
$Comp
L Mechanical:MountingHole_Pad H3
U 1 1 60D6B05B
P 13200 8950
F 0 "H3" V 13154 9100 50  0000 L CNN
F 1 "MountingHole_Pad" V 13245 9100 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.5mm_Pad" H 13200 8950 50  0001 C CNN
F 3 "~" H 13200 8950 50  0001 C CNN
	1    13200 8950
	0    1    1    0   
$EndComp
$Comp
L Mechanical:MountingHole_Pad H5
U 1 1 60D6B4F1
P 13200 9350
F 0 "H5" V 13154 9500 50  0000 L CNN
F 1 "MountingHole_Pad" V 13245 9500 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.5mm_Pad" H 13200 9350 50  0001 C CNN
F 3 "~" H 13200 9350 50  0001 C CNN
	1    13200 9350
	0    1    1    0   
$EndComp
Wire Wire Line
	13100 9350 13000 9350
Connection ~ 13000 9350
Wire Wire Line
	13000 9350 13000 9550
$Comp
L Mechanical:MountingHole_Pad H6
U 1 1 60D9AE58
P 13200 9550
F 0 "H6" V 13154 9700 50  0000 L CNN
F 1 "MountingHole_Pad" V 13245 9700 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.5mm_Pad" H 13200 9550 50  0001 C CNN
F 3 "~" H 13200 9550 50  0001 C CNN
	1    13200 9550
	0    1    1    0   
$EndComp
Wire Wire Line
	13100 9550 13000 9550
Connection ~ 13000 9550
Wire Wire Line
	13000 9550 13000 9650
$Comp
L power:GND #PWR0148
U 1 1 60DA739E
P 13000 9650
F 0 "#PWR0148" H 13000 9400 50  0001 C CNN
F 1 "GND" H 13005 9477 50  0000 C CNN
F 2 "" H 13000 9650 50  0001 C CNN
F 3 "" H 13000 9650 50  0001 C CNN
	1    13000 9650
	1    0    0    -1  
$EndComp
Wire Wire Line
	13100 8950 13000 8950
Connection ~ 13000 8950
Wire Wire Line
	13100 8750 13000 8750
Wire Wire Line
	13000 8750 13000 8950
Text GLabel 5650 1350 2    50   Input ~ 0
D7
Text GLabel 5650 1450 2    50   Input ~ 0
D6
Text GLabel 5650 1550 2    50   Input ~ 0
D5
Text GLabel 5650 1650 2    50   Input ~ 0
D4
Text GLabel 5650 1750 2    50   Input ~ 0
D3
Text GLabel 5650 1850 2    50   Input ~ 0
D2
Text GLabel 5650 1950 2    50   Input ~ 0
D1
Text GLabel 5650 2050 2    50   Input ~ 0
D0
Text GLabel 5650 4250 2    50   Input ~ 0
A00
Wire Wire Line
	5550 1350 5650 1350
Wire Wire Line
	5550 1450 5650 1450
Wire Wire Line
	5650 1550 5550 1550
Wire Wire Line
	5550 1650 5650 1650
Wire Wire Line
	5550 1750 5650 1750
Wire Wire Line
	5550 1850 5650 1850
Wire Wire Line
	5550 1950 5650 1950
Wire Wire Line
	5650 2050 5550 2050
Wire Wire Line
	5550 2350 5650 2350
Wire Wire Line
	5550 2450 5650 2450
Wire Wire Line
	5650 2550 5550 2550
Wire Wire Line
	5550 2650 5650 2650
Wire Wire Line
	5550 2750 5650 2750
Wire Wire Line
	5550 2950 5650 2950
Wire Wire Line
	5650 3050 5550 3050
Wire Wire Line
	5550 3150 5650 3150
Wire Wire Line
	5550 3250 5650 3250
Wire Wire Line
	5550 3350 5650 3350
Wire Wire Line
	5550 3450 5650 3450
Wire Wire Line
	5650 3550 5550 3550
Wire Wire Line
	5550 3650 5650 3650
Wire Wire Line
	5550 3750 5650 3750
Wire Wire Line
	5550 3850 5650 3850
Wire Wire Line
	5550 3950 5650 3950
Wire Wire Line
	5650 4050 5550 4050
Wire Wire Line
	5550 4150 5650 4150
Wire Wire Line
	5550 4250 5650 4250
Text GLabel 5650 4150 2    50   Input ~ 0
A01
Text GLabel 5650 4050 2    50   Input ~ 0
A02
Text GLabel 5650 3950 2    50   Input ~ 0
A03
Text GLabel 5650 3550 2    50   Input ~ 0
A07
Text GLabel 5650 3450 2    50   Input ~ 0
A08
Text GLabel 5650 3350 2    50   Input ~ 0
A09
Text GLabel 5650 3250 2    50   Input ~ 0
A10
Text GLabel 5650 3150 2    50   Input ~ 0
A11
Text GLabel 5650 3050 2    50   Input ~ 0
A12
Text GLabel 5650 2950 2    50   Input ~ 0
A13
Text GLabel 5650 3650 2    50   Input ~ 0
A06
Text GLabel 5650 3750 2    50   Input ~ 0
A05
Text GLabel 5650 3850 2    50   Input ~ 0
A04
Text GLabel 5650 2350 2    50   Input ~ 0
A19
Text GLabel 5650 2450 2    50   Input ~ 0
A18
Text GLabel 5650 2550 2    50   Input ~ 0
A17
Text GLabel 5650 2650 2    50   Input ~ 0
A16
Text GLabel 5650 2750 2    50   Input ~ 0
A15
NoConn ~ 5550 1250
Text GLabel 5650 2850 2    50   Input ~ 0
A14
Wire Wire Line
	5550 2850 5650 2850
$Comp
L 00Common:Bus_Tandy_Plus_For_Pass_Through J200
U 1 1 60D64444
P 5050 2750
F 0 "J200" H 5050 4517 50  0000 C CNN
F 1 "Bus_Tandy_Plus_Through" H 5050 4426 50  0000 C CNN
F 2 "00Custom:TandyPlusSocket" H 5050 2750 50  0001 C CNN
F 3 "https://en.wikipedia.org/wiki/Industry_Standard_Architecture" H 5050 2750 50  0001 C CNN
	1    5050 2750
	1    0    0    -1  
$EndComp
Text GLabel 5650 2250 2    50   Input ~ 0
AEN
Wire Wire Line
	5650 2250 5550 2250
NoConn ~ 4550 1650
$Comp
L power:GND #PWR0149
U 1 1 60DCCE6A
P 4450 4450
F 0 "#PWR0149" H 4450 4200 50  0001 C CNN
F 1 "GND" H 4455 4277 50  0000 C CNN
F 2 "" H 4450 4450 50  0001 C CNN
F 3 "" H 4450 4450 50  0001 C CNN
	1    4450 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	4550 1250 4450 1250
Wire Wire Line
	4450 1250 4450 2150
Wire Wire Line
	4550 4250 4450 4250
Connection ~ 4450 4250
Wire Wire Line
	4450 4250 4450 4450
Wire Wire Line
	4550 2150 4450 2150
Connection ~ 4450 2150
Wire Wire Line
	4450 2150 4450 4250
$Comp
L power:VCC #PWR0166
U 1 1 60E02A45
P 4350 1200
F 0 "#PWR0166" H 4350 1050 50  0001 C CNN
F 1 "VCC" H 4365 1373 50  0000 C CNN
F 2 "" H 4350 1200 50  0001 C CNN
F 3 "" H 4350 1200 50  0001 C CNN
	1    4350 1200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4350 1200 4350 1450
Wire Wire Line
	4350 1450 4550 1450
Wire Wire Line
	4350 1450 4350 4050
Wire Wire Line
	4350 4050 4550 4050
Connection ~ 4350 1450
$Comp
L power:-12V #PWR0168
U 1 1 60E2855D
P 4200 4450
F 0 "#PWR0168" H 4200 4550 50  0001 C CNN
F 1 "-12V" H 4215 4623 50  0000 C CNN
F 2 "" H 4200 4450 50  0001 C CNN
F 3 "" H 4200 4450 50  0001 C CNN
	1    4200 4450
	-1   0    0    1   
$EndComp
Wire Wire Line
	4200 4450 4200 1850
Wire Wire Line
	4200 1850 4550 1850
$Comp
L power:+12V #PWR0169
U 1 1 60E3BD43
P 4100 1200
F 0 "#PWR0169" H 4100 1050 50  0001 C CNN
F 1 "+12V" H 4115 1373 50  0000 C CNN
F 2 "" H 4100 1200 50  0001 C CNN
F 3 "" H 4100 1200 50  0001 C CNN
	1    4100 1200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4100 1200 4100 2050
Wire Wire Line
	4100 2050 4550 2050
Text GLabel 5650 2150 2    50   Input ~ 0
RDYIN
Wire Wire Line
	5650 2150 5550 2150
Wire Wire Line
	1600 2250 1100 2250
Wire Wire Line
	1600 2350 1100 2350
Wire Wire Line
	1600 2450 1100 2450
Wire Wire Line
	1600 2550 1100 2550
Text GLabel 1100 2250 0    39   Input ~ 0
~MEMW
Text GLabel 1100 2350 0    39   Input ~ 0
~MEMR
Text GLabel 1100 2450 0    39   Input ~ 0
~IOW
Text GLabel 1100 2550 0    39   Input ~ 0
~IOR
Text GLabel 1050 1350 0    50   Input ~ 0
RESET
Text GLabel 1100 3550 0    50   Input ~ 0
IRQ4
Wire Wire Line
	1600 3550 1100 3550
Text GLabel 1100 3650 0    50   Input ~ 0
IRQ3
Wire Wire Line
	1600 3650 1100 3650
Text GLabel 1050 1550 0    50   Input ~ 0
IRQ2
Text GLabel 1050 1750 0    50   Input ~ 0
FDCDRQ
Text GLabel 1050 1950 0    50   Input ~ 0
AUDIOIN
Text GLabel 1100 2650 0    39   Input ~ 0
~DACK3
Text GLabel 1100 2850 0    39   Input ~ 0
~DACK1
Text GLabel 1100 2750 0    50   Input ~ 0
DRQ3
Text GLabel 1100 2950 0    50   Input ~ 0
DRQ1
Text GLabel 1100 3050 0    39   Input ~ 0
~REFRESH
Text GLabel 1100 3150 0    50   Input ~ 0
CLK
Text GLabel 1100 3250 0    39   Input ~ 0
~RFSH~_(IRQ7)
Text GLabel 1100 3350 0    39   Input ~ 0
~BREQ~_(IRQ6)
Text GLabel 1100 3450 0    50   Input ~ 0
NC_(IRQ5)
Text GLabel 1100 3750 0    39   Input ~ 0
~FDCACK
Text GLabel 1100 3850 0    50   Input ~ 0
DMATC
Text GLabel 1100 3950 0    50   Input ~ 0
ALE
Text GLabel 1100 4150 0    50   Input ~ 0
OSC
Wire Wire Line
	1100 4150 1600 4150
Wire Wire Line
	1100 3950 1600 3950
Wire Wire Line
	1600 3850 1100 3850
Wire Wire Line
	1100 3750 1600 3750
Wire Wire Line
	1600 3450 1100 3450
Wire Wire Line
	1100 3350 1600 3350
Wire Wire Line
	1600 3250 1100 3250
Wire Wire Line
	1100 3150 1600 3150
Wire Wire Line
	1600 3050 1100 3050
Wire Wire Line
	1100 2950 1600 2950
Wire Wire Line
	1600 2850 1100 2850
Wire Wire Line
	1100 2750 1600 2750
Wire Wire Line
	1600 2650 1100 2650
Wire Wire Line
	1050 1950 1600 1950
Wire Wire Line
	1600 1750 1050 1750
Wire Wire Line
	1050 1550 1600 1550
Text Notes 3750 750  0    79   ~ 0
TANDY PLUS BUS THROUGH
Wire Notes Line
	3450 600  6150 600 
Wire Notes Line
	6150 600  6150 4950
Wire Notes Line
	6150 4950 3450 4950
Wire Notes Line
	3450 4950 3450 600 
Wire Wire Line
	1050 1350 1600 1350
Wire Wire Line
	4550 2250 4050 2250
Wire Wire Line
	4550 2350 4050 2350
Wire Wire Line
	4550 2450 4050 2450
Wire Wire Line
	4550 2550 4050 2550
Text GLabel 4050 2250 0    39   Input ~ 0
~MEMW
Text GLabel 4050 2350 0    39   Input ~ 0
~MEMR
Text GLabel 4050 2450 0    39   Input ~ 0
~IOW
Text GLabel 4050 2550 0    39   Input ~ 0
~IOR
Text GLabel 4000 1350 0    50   Input ~ 0
RESET
Text GLabel 4050 3550 0    50   Input ~ 0
IRQ4
Wire Wire Line
	4550 3550 4050 3550
Text GLabel 4050 3650 0    50   Input ~ 0
IRQ3
Wire Wire Line
	4550 3650 4050 3650
Text GLabel 4000 1550 0    50   Input ~ 0
IRQ2
Text GLabel 4000 1750 0    50   Input ~ 0
FDCDRQ
Text GLabel 4000 1950 0    50   Input ~ 0
AUDIOIN
Text GLabel 4050 2650 0    39   Input ~ 0
~DACK3
Text GLabel 4050 2850 0    39   Input ~ 0
~DACK1
Text GLabel 4050 2750 0    50   Input ~ 0
DRQ3
Text GLabel 4050 2950 0    50   Input ~ 0
DRQ1
Text GLabel 4050 3050 0    39   Input ~ 0
~REFRESH
Text GLabel 4050 3150 0    50   Input ~ 0
CLK
Text GLabel 4050 3250 0    39   Input ~ 0
~RFSH~_(IRQ7)
Text GLabel 4050 3350 0    39   Input ~ 0
~BREQ~_(IRQ6)
Text GLabel 4050 3450 0    50   Input ~ 0
NC_(IRQ5)
Text GLabel 4050 3750 0    39   Input ~ 0
~FDCACK
Text GLabel 4050 3850 0    50   Input ~ 0
DMATC
Text GLabel 4050 3950 0    50   Input ~ 0
ALE
Text GLabel 4050 4150 0    50   Input ~ 0
OSC
Wire Wire Line
	4050 4150 4550 4150
Wire Wire Line
	4050 3950 4550 3950
Wire Wire Line
	4550 3850 4050 3850
Wire Wire Line
	4050 3750 4550 3750
Wire Wire Line
	4550 3450 4050 3450
Wire Wire Line
	4050 3350 4550 3350
Wire Wire Line
	4550 3250 4050 3250
Wire Wire Line
	4050 3150 4550 3150
Wire Wire Line
	4550 3050 4050 3050
Wire Wire Line
	4050 2950 4550 2950
Wire Wire Line
	4550 2850 4050 2850
Wire Wire Line
	4050 2750 4550 2750
Wire Wire Line
	4550 2650 4050 2650
Wire Wire Line
	4000 1950 4550 1950
Wire Wire Line
	4550 1750 4000 1750
Wire Wire Line
	4000 1550 4550 1550
Wire Wire Line
	4000 1350 4550 1350
Text GLabel 2700 2150 2    50   Input ~ 0
RDYIN
Wire Wire Line
	2600 2150 2700 2150
Wire Wire Line
	13000 8950 13000 9350
$Comp
L Switch:SW_DIP_x10 SW1
U 1 1 60DCA93E
P 10700 9900
F 0 "SW1" H 10700 10667 50  0000 C CNN
F 1 "SW_DIP_x10" H 10700 10576 50  0000 C CNN
F 2 "Button_Switch_THT:SW_DIP_SPSTx10_Slide_9.78x27.58mm_W7.62mm_P2.54mm" H 10700 9900 50  0001 C CNN
F 3 "~" H 10700 9900 50  0001 C CNN
	1    10700 9900
	1    0    0    -1  
$EndComp
Text GLabel 10400 10300 0    39   Input ~ 0
~CE~_BIOS_DECODE
Text GLabel 11000 10300 2    39   Input ~ 0
~CE~_BIOS_THRU
Wire Wire Line
	11000 10100 11050 10100
Wire Wire Line
	11000 10200 11050 10200
Wire Wire Line
	11050 10200 11050 10100
Connection ~ 11050 10100
Wire Wire Line
	11050 10100 11100 10100
Text GLabel 10100 1700 0    50   Input ~ 0
D7_BUF
Text GLabel 10100 1800 0    50   Input ~ 0
D6_BUF
Text GLabel 10100 1900 0    50   Input ~ 0
D5_BUF
Text GLabel 10100 2000 0    50   Input ~ 0
D4_BUF
Text GLabel 10100 2100 0    50   Input ~ 0
D3_BUF
Text GLabel 10100 2200 0    50   Input ~ 0
D2_BUF
Text GLabel 10100 2300 0    50   Input ~ 0
D1_BUF
Text GLabel 10100 2400 0    50   Input ~ 0
D0_BUF
Text GLabel 11100 2400 2    50   Input ~ 0
D0
Text GLabel 11100 2300 2    50   Input ~ 0
D1
Text GLabel 11100 2200 2    50   Input ~ 0
D2
Text GLabel 11100 2100 2    50   Input ~ 0
D3
Text GLabel 11100 2000 2    50   Input ~ 0
D4
Text GLabel 11100 1900 2    50   Input ~ 0
D5
Text GLabel 11100 1800 2    50   Input ~ 0
D6
Text GLabel 11100 1700 2    50   Input ~ 0
D7
NoConn ~ 3150 10550
NoConn ~ 3350 10550
NoConn ~ 3450 10550
NoConn ~ 4150 10550
Text Notes 3350 11050 0    50   ~ 0
Connections to N/C pins\nare to assist with routing.
$Comp
L power:VCC #PWR0170
U 1 1 6103CF73
P 2500 10650
F 0 "#PWR0170" H 2500 10500 50  0001 C CNN
F 1 "VCC" H 2515 10823 50  0000 C CNN
F 2 "" H 2500 10650 50  0001 C CNN
F 3 "" H 2500 10650 50  0001 C CNN
	1    2500 10650
	1    0    0    -1  
$EndComp
Wire Wire Line
	3250 10550 3250 11000
Wire Wire Line
	3250 11000 2500 11000
Wire Wire Line
	2500 11000 2500 10650
Wire Wire Line
	3950 10550 3950 10750
Wire Wire Line
	3950 10750 5350 10750
Wire Wire Line
	5350 10750 5350 9150
Connection ~ 5350 9150
Wire Wire Line
	4050 10550 4050 10650
Wire Wire Line
	4050 10650 5250 10650
Wire Wire Line
	5250 10650 5250 9050
Connection ~ 5250 9050
Text GLabel 3850 10550 3    50   Input ~ 0
RESET
$EndSCHEMATC
